...
<button
	class="absolute top-4 {isCollapsed
		? ' translate-x-2'
		...
		<ul class="space-y-5 font-medium">
			{#if role === 'Doctor'}
				...
				>
					<a
						...
						class="text-body rounded-base hover:bg-neutral-tertiary hover:text-fg-brand group flex items-center px-2 py-1.5 {isCollapsed
							...
							: 'pl-4'} "
					>
						<HeartPulse color={page.url.pathname.includes('/app/patients') ? '#1754a7' : 'white'} />
						{#if !isCollapsed}
							<span
				...
				>
					...
					>
						<CalendarRange
							color={page.url.pathname.includes('/app/reservations/doctor') ? '#1754a7' : 'white'}
						/>
						{#if !isCollapsed}
							<span
...
<div class="p-4 {isCollapsed ? 'ml-12' : 'ml-64'} transition-all duration-300">
