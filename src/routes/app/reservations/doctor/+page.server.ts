import { ReservationRepositoryImpl } from "$lib/server/services/ReservationServiceImpl";
import type { PageServerLoad } from "./$types";
import { toPOJO } from "$lib/shared/utils/Utils";

export const load: PageServerLoad = async ({ locals }) => {
  try {
    const listReservations = await new ReservationRepositoryImpl().listReservations();
